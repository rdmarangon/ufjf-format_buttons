...
class content extends content_base
{
    ...
    public function export_for_template(\renderer_base $output): \stdClass
    {
        ...
        $numsections_option = $DB->get_field('course_format_options', 'value',
            ['courseid' => $course->id, 'format' => 'buttons', 'sectionid' => 0, 'name' => 'numsections']);
        $numsections = $numsections_option !== false ? (int)$numsections_option : $format->get_last_section_number();
        foreach ($all_sections as $section) {
            ...
            $info->sectionnumber = $section->section;
        ...
        if ($section_select > $numsections) {
            $section_select = "";
            self::save_last_section_access($course->id, null);
        }

        if ($section_prev > $numsections) {
            $section_prev = null;
            self::save_last_section_access($course->id, null);
        }
        ...
        if ($section_select != "") {
            if ($section_select == "0" && !$course->section_zero_ubication) {
                ...
                if (isset($array_sections[$section_select])) {
        ...
        $colorcurrent_raw = $DB->get_field('course_format_options', 'value',
            ['courseid' => $course->id, 'format' => 'buttons', 'sectionid' => 0, 'name' => 'colorcurrent']);
        if (!empty($colorcurrent_raw)) {
            $colorcurrent = (strpos($colorcurrent_raw, '#') === 0) ? $colorcurrent_raw : '#' . $colorcurrent_raw;
        } else {
            $colorcurrent = '#115898';
        }

        $marker = !empty($course->marker) ? (int)$course->marker : 0;

        foreach ($array_sections as &$info) {
            if (empty($info->body)) {
                continue;
            }
            $is_hidden_section = !empty($info->is_hidden);
            $is_selected = !empty($info->selected);
            $is_current_section = $marker > 0 && (int)($info->sectionnumber ?? 0) === $marker;
            if ($is_hidden_section) {
                if ($isteacher) {
                    $info->hidden_teacher_selected = $is_selected ? true : null;
                    $info->hidden_teacher_normal = $is_selected ? null : true;
                } else {
                    $info->hidden_student = true;
                }
            } else {
                if ($is_selected) {
                    $info->visible_selected = true;
                } else if ($is_current_section) {
                    $info->visible_current = true;
                } else {
                    $info->visible_normal = true;
                }
            }
        }
        unset($info);
